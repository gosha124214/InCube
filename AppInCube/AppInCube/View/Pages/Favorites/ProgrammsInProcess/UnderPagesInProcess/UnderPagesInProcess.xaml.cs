...
namespace AppInCube.View.Pages.Favorites.ProgrammsInProcess.UnderPagesInProcess
{
    public partial class UnderPagesInProcess : ContentPage
    {
        ...
        public UnderPagesInProcess(SQLliteTableParty party)
        {
            ...
            // Загружаем статус уведомлений при загрузке страницы
            LoadNotificationsStatus();
        }

        // Метод для загрузки статуса уведомлений
        private async void LoadNotificationsStatus()
        {
            try
            {
                // Блокируем Switch во время проверки
                NotificationsSwitch.IsEnabled = false;
                NotificationsStatusLabel.Text = "Проверка уведомлений...";
                ...
                if (notificationService != null)
                {
                    ...
                    // Фильтруем уведомления для этой партии
                    var partyNotifications = allNotifications
                        .Where(n => n.Title.Contains($"PID{currentParty.IdParty}") ||
                                   n.Message.Contains($"Программа #{currentParty.IdParty}"))
                        .ToList();

                    // Если есть хотя бы одно уведомление - включаем Switch
                    ...
                    if (hasNotifications)
                    {
                        ...
                        string statusText = $"Включено ({notificationCount} уведомлений)";
                        if (nextNotification != null)
                        {
                            statusText += $"\nСледующее: {nextNotification.ScheduleTime:HH:mm}";
                        }
                    ...
                    {
                        NotificationsStatusLabel.Text = "Выключено";
                    }
                }
                else
                {
                    NotificationsStatusLabel.Text = "Сервис уведомлений недоступен";
                    NotificationsSwitch.IsEnabled = false;
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Ошибка при загрузке статуса уведомлений: {ex.Message}");
                NotificationsStatusLabel.Text = "Ошибка загрузки";
            }
            finally
            {
        ...
        private async void OnNotificationsSwitchToggled(object sender, ToggledEventArgs e)
        {
            ...
            try
            {
                ...
                if (notificationService == null)
                {
                    await DisplayAlert("Ошибка", "Сервис уведомлений недоступен", "OK");
                    NotificationsSwitch.IsToggled = !e.Value; // Возвращаем предыдущее состояние
                    return;
                }

                if (e.Value) // Если Switch включили
                {
                    ...
                    if (!hasPermission)
                    {
                        await DisplayAlert("Внимание",
                            "Для работы уведомлений нужны разрешения. " +
                    ...
                    // Создаем уведомления для партии
                    bool success = await CreatePartyNotifications();

                    if (success)
                    {
                        NotificationsStatusLabel.Text = "Уведомления включены";
                        ShowMessage("✅ Уведомления включены", true);
                    }
                    else
                    {
                        NotificationsSwitch.IsToggled = false;
                        NotificationsStatusLabel.Text = "Ошибка создания";
                        ShowMessage("❌ Не удалось создать уведомления", false);
                    }
                }
                else // Если Switch выключили
                {
                    // Удаляем все уведомления партии
                    await CancelPartyNotifications();
                    NotificationsStatusLabel.Text = "Уведомления выключены";
                    ShowMessage("🔕 Уведомления выключены", true);
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Ошибка при переключении уведомлений: {ex.Message}");
                NotificationsSwitch.IsToggled = !e.Value; // Возвращаем предыдущее состояние
                NotificationsStatusLabel.Text = "Ошибка";
        ...
        // Метод для создания уведомлений партии
            ...
            try
            {
                    ...
                    if (ParametersDiffer(sortedDays[i - 1], sortedDays[i]))
                    {
                        ...
                        if (notificationTime > DateTime.Now)
                        {
                            var success = await notificationService.ScheduleExactNotificationAsync(
                                ...
                                $"Программа #{currentParty.IdParty}, День {sortedDays[i].Day}",
                                notificationTime);
                ...
                if (endTime > DateTime.Now)
                {
                    var success = await notificationService.ScheduleExactNotificationAsync(
                        ...
                        $"Программа #{currentParty.IdParty} завершена!",
                        endTime);
                ...
                // Обновляем статус
                NotificationsStatusLabel.Text = $"Включено ({createdCount} уведомлений)";
                Console.WriteLine($"✅ Создано {createdCount} уведомлений для партии {currentParty.IdParty}");
        ...
        // Метод для отмены уведомлений партии
        private async Task CancelPartyNotifications()
        {
            try
            {
                ...
                if (notificationService != null)
                {
                    ...
                    Console.WriteLine($"✅ Удалено {partyNotifications.Count} уведомлений для партии {currentParty.IdParty}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"❌ Ошибка при удалении уведомлений: {ex.Message}");
            }
        }
        ...
        // Обновляем метод OnAppearing для проверки статуса уведомлений при возвращении на страницу
        protected override async void OnAppearing()
        {
            base.OnAppearing();
            // Обновляем статус уведомлений при каждом появлении страницы
            await Task.Delay(500); // Небольшая задержка для стабильности
            LoadNotificationsStatus();
        }

        // Остальные методы остаются без изменений (сохраняем всю существующую логику)
